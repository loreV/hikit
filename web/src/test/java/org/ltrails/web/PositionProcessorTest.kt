package org.ltrails.web

import mil.nga.sf.geojson.FeatureConverter
import org.junit.Assert.assertEquals
import org.junit.Assert.fail
import org.junit.Test
import org.ltrails.common.data.Coordinates
import org.ltrails.common.data.Position
import org.mockito.Mockito
import org.mockito.Mockito.`when`
import org.mockito.Mockito.mock

class PositionHelperTest {

    private val fourPointHike = "{\"type\":\"FeatureCollection\",\"features\":[{\"type\":\"Feature\",\"properties\":{},\"geometry\":{\"type\":\"LineString\",\"coordinates\":[[11.244678497314451,44.49154610207528,1],[11.251072883605957,44.48923471237884,1],[11.255149841308594,44.4873059329883,1],[11.259291172027588,44.48432079153089,1]]}}]}"

    @Test
    fun `given a simple two point hike, should calculate distance from start`() {
        val shortHike = "{\"type\":\"FeatureCollection\",\"features\":[{\"type\":\"Feature\",\"properties\":{},\"geometry\":{\"type\":\"LineString\",\"coordinates\":[[11.245107650756836,44.491423646640975,0],[11.251587867736815,44.4890050991292,0]]}}]}"
        val toGeoJsonObject = FeatureConverter.toGeoJsonObject(shortHike)

        // Start pos
        val mockStartPosition = mock(Position::class.java)
        val mockCoords = mock(Coordinates::class.java)

        `when`(mockStartPosition.coords).thenReturn(mockCoords)
        `when`(mockCoords.latitude).thenReturn(44.491423646640975)
        `when`(mockCoords.longitude).thenReturn(11.245107650756836)

        // Destination
        val mockConnectingPosition = mock(Position::class.java)
        val mockCoordsConnecting = mock(Coordinates::class.java)

        `when`(mockConnectingPosition.coords).thenReturn(mockCoordsConnecting)
        `when`(mockCoordsConnecting.latitude).thenReturn(44.4890050991292)
        `when`(mockCoordsConnecting.longitude).thenReturn(11.251587867736815)

        val distanceBetweenPointsOnTrail = PositionHelper().getDistanceBetweenPointsOnTrailInM(toGeoJsonObject, mockStartPosition, mockConnectingPosition)
        assertEquals(580.78, distanceBetweenPointsOnTrail, 1.0)
    }

    @Test
    fun `given a simple four point hike, should calculate distance from start`() {
        val toGeoJsonObject = FeatureConverter.toGeoJsonObject(fourPointHike)

        // Start pos
        val mockStartPosition = mock(Position::class.java)
        val mockCoords = mock(Coordinates::class.java)

        `when`(mockStartPosition.coords).thenReturn(mockCoords)
        `when`(mockCoords.latitude).thenReturn(44.49154610207528)
        `when`(mockCoords.longitude).thenReturn(11.244678497314451)

        // Destination
        val mockConnectingPosition = mock(Position::class.java)
        val mockCoordsConnecting = mock(Coordinates::class.java)

        `when`(mockConnectingPosition.coords).thenReturn(mockCoordsConnecting)
        `when`(mockCoordsConnecting.latitude).thenReturn(44.48432079153089)
        `when`(mockCoordsConnecting.longitude).thenReturn(11.259291172027588)

        val distanceBetweenPointsOnTrail = PositionHelper().getDistanceBetweenPointsOnTrailInM(toGeoJsonObject, mockStartPosition, mockConnectingPosition)
        assertEquals(1425.30, distanceBetweenPointsOnTrail, 1.0)
    }

    @Test
    fun `given a simple four point hike, should calculate distance from middle`() {
        val toGeoJsonObject = FeatureConverter.toGeoJsonObject(fourPointHike)

        // Start pos
        val mockStartPosition = mock(Position::class.java)
        val mockCoords = mock(Coordinates::class.java)

        `when`(mockStartPosition.coords).thenReturn(mockCoords)
        `when`(mockCoords.latitude).thenReturn(44.48923471237884)
        `when`(mockCoords.longitude).thenReturn(11.251072883605957)
        // Destination
        val mockConnectingPosition = mock(Position::class.java)
        val mockCoordsConnecting = mock(Coordinates::class.java)

        `when`(mockConnectingPosition.coords).thenReturn(mockCoordsConnecting)
        `when`(mockCoordsConnecting.latitude).thenReturn(44.48432079153089)
        `when`(mockCoordsConnecting.longitude).thenReturn(11.259291172027588)

        val distanceBetweenPointsOnTrail = PositionHelper().getDistanceBetweenPointsOnTrailInM(toGeoJsonObject, mockStartPosition, mockConnectingPosition)
        assertEquals(856.00, distanceBetweenPointsOnTrail, 1.0)
    }

    @Test
    fun `given a simple four point hike, should calculate 0 distance for same point`() {
        val toGeoJsonObject = FeatureConverter.toGeoJsonObject(fourPointHike)

        // Start pos
        val mockStartPosition = mock(Position::class.java)
        val mockCoords = mock(Coordinates::class.java)

        `when`(mockStartPosition.coords).thenReturn(mockCoords)
        `when`(mockCoords.latitude).thenReturn(44.48432079153089) // Same as connecting point
        `when`(mockCoords.longitude).thenReturn(11.259291172027588) // Same as connecting point
        // Destination
        val mockConnectingPosition = mock(Position::class.java)
        val mockCoordsConnecting = mock(Coordinates::class.java)

        `when`(mockConnectingPosition.coords).thenReturn(mockCoordsConnecting)
        `when`(mockCoordsConnecting.latitude).thenReturn(44.48432079153089)
        `when`(mockCoordsConnecting.longitude).thenReturn(11.259291172027588)

        val distanceBetweenPointsOnTrail = PositionHelper().getDistanceBetweenPointsOnTrailInM(toGeoJsonObject, mockStartPosition, mockConnectingPosition)
        assertEquals(0.0, distanceBetweenPointsOnTrail, 1.0)
    }

    @Test
    fun `given a simple four point hike, should throw exception cause points do not exist`() {
        val toGeoJsonObject = FeatureConverter.toGeoJsonObject(fourPointHike)

        // Start pos
        val mockStartPosition = mock(Position::class.java)
        val mockCoords = mock(Coordinates::class.java)

        `when`(mockStartPosition.coords).thenReturn(mockCoords)
        `when`(mockCoords.latitude).thenReturn(44.4843207915308) // Not existing!
        `when`(mockCoords.longitude).thenReturn(11.25929117202758) // Not existing!
        // Destination
        val anyMock = mock(Position::class.java)
        val anyCoord = mock(Coordinates::class.java)

        Mockito.`when`(anyMock.coords).thenReturn(anyCoord)
        Mockito.`when`(anyCoord.latitude).thenReturn(44.48432079153089)
        Mockito.`when`(anyCoord.longitude).thenReturn(11.259291172027588)

        try {
            PositionHelper().getDistanceBetweenPointsOnTrailInM(toGeoJsonObject, mockStartPosition, anyMock)
            fail()
        } catch (e: IllegalStateException) {
        }
    }
}